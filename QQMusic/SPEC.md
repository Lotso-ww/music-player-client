...
- [ ] `P0-01` 防御播放列表 `currentIndexChanged(-1)`，消除负索引访问崩溃。
- [ ] `P0-02` 歌词解析前清空旧数据，无歌词或打开失败时显示明确空状态。
- [ ] `P0-03` 修复 LRC 小数秒/毫秒解析，覆盖 1、2、3 位小数和异常时间标签。
- [ ] `P0-04` 移除元数据解析的无限事件循环等待，改为可超时、可失败的异步流程。
- [ ] `P0-05` 初始化 `totalTime`、`isDrag`、`musicDuration` 等状态，消除未定义行为。
- [ ] `P0-06` 让 SQLite 初始化显式返回结果；打开失败时停止依赖数据库的后续初始化。
- [ ] `P0-07` 使用稳定的应用数据目录保存数据库，不依赖进程当前工作目录。
- [ ] `P0-08` 统一退出与持久化路径，覆盖托盘退出、窗口关闭和 Alt+F4。
- [ ] `P0-09` 检查 `QSharedMemory::create()` 结果并记录单实例初始化错误。
- [ ] `P0-10` 修正托盘等 Qt 资源路径不一致问题。
- [ ] `P0-11` 修复重复导入后播放列表重复追加的问题。
- [ ] `P0-12` 数据库批量保存使用事务，失败时回滚并报告。
- [ ] `P0-13` 将同步 `repaint()` 等潜在阻塞刷新改为合适的事件循环更新方式。
- [ ] `P0-14` 修复推荐分页丢弃尾项等边界问题。
- [ ] `P0-15` 为进度比例、零时长、空列表、失效 URL 等输入增加边界保护。
- [ ] `P0-16` 建立最小测试工程、构建说明、日志规范和模块回顾目录。
...
- [ ] `P1A-01` 支持按歌曲名、歌手、专辑进行不区分大小写的模糊搜索。
- [ ] `P1A-02` 支持清空、无结果、输入法组合和快速连续输入。
- [ ] `P1A-03` 万级数据搜索目标 P95 小于 150 ms，不阻塞播放。
- [ ] `P1A-04` 搜索结果可直接播放、收藏和定位本地文件状态。

#### P1B 主题换肤

- [ ] `P1B-01` 新增主题管理器，集中管理 QSS、图标和资源。
- [ ] `P1B-02` 游客可换肤，选择写入本地设置。
- [ ] `P1B-03` 登录后主题设置可同步，但同步失败不影响本地主题。
- [ ] `P1B-04` 至少覆盖默认主题和一个完整备选主题，无局部样式残留。
...
- [ ] `P1D-01` 支持导入文件和目录，扫描 MP3、FLAC、WAV、AAC/M4A。
- [ ] `P1D-02` 文件遍历和元数据解析进入 Worker，不阻塞 UI。
- [ ] `P1D-03` 支持进度、取消、超时、失败汇总和重复文件提示。
- [ ] `P1D-04` 按路径与内容特征去重；文件移动后尝试重新关联。
- [ ] `P1D-05` 文件删除后标记不可用，不立即删除收藏和历史。
- [ ] `P1D-06` 子线程不操作 UI，数据库连接不跨线程共享。
...
- [ ] 记录当前版本的可复现构建环境与运行截图。
- [ ] 按风险从 `P0-01`、`P0-02`、`P0-04`、`P0-05` 开始修复。
- [ ] 每个修复完成后立即测试、提交和填写简短回顾，不等待 P0 全部完成后补记。
