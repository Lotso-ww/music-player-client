...
- Docker 首次构建日志确认 Drogon 使用 Git 子模块提供 Trantor；Dockerfile 已改为递归浅克隆。Ubuntu 24.04 上 Drogon 未自动发现已安装的 MySQL 开发包，Dockerfile 已显式指定其头文件和库路径。需使用这些修复重新构建后再记录正式验收结果。
